...
export class ProblemsService {
  ...
  async create(dto: CreateProblemDto) { const problem = await this.prisma.problem.create({ data: this.toCreateData(dto), include: detailInclude }); await this.invalidateDashboard(); return problem; }
  async update(id: number, dto: UpdateProblemDto) {
    ...
    const problem = await this.prisma.problem.update({ where: { id }, data, include: detailInclude });
    await this.invalidateDashboard(); return problem;
  }
  ...
  async addSolution(problemId: number, dto: CreateSolutionDto) { await this.findOne(problemId); const solution = await this.prisma.solution.create({ data: { problemId, ...dto } }); await this.invalidateDashboard(); return solution; }
  ...
  async seedImport(dto: SeedImportDto) { const created = await this.prisma.$transaction(dto.problems.map((problem) => this.prisma.problem.create({ data: this.toCreateData(problem) }))); await this.invalidateDashboard(); return { created: created.length }; }
